...
@Component({
    selector: 'in-queue-tasks',
    styleUrls: ['../styles/task.scss'],
    templateUrl: '../views/in-queue-tasks.component.html',
})

export class InQueueTasksComponent implements OnInit {
    ...
    ngOnInit() {
        this.tasks$ = this.taskStore.select('tasks')
            .filter(Boolean)
            .map(tasks => tasks.filter(task => task.status === 'in_queue'));
    }
